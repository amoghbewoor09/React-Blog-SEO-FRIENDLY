...
export default function CardbContent() {
  const { lastUpdatedC, setLastUpdatedC } = useContext(DateTimeContext);

  const updateTimeAndDateC = () => {
    const currentDateTime = moment().format('YYYY-MM-DD HH:mm:ss');
    setLastUpdatedC(currentDateTime);
  };
  ...
  return (
    <>
        ...
        <Helmet>
        <title>Card c Content - Lorem Ipsum | Publisher Name</title>
      ...
      <div className="blog-container" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '3rem' }}>
        <div className="header" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {lastUpdatedC && (<Typography variant="body2" color="primary"> Last Updated: {lastUpdatedC}</Typography>)}
        ...
        <div className="content" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Button variant="contained" style={{ marginTop: '2rem' }} onClick={updateTimeAndDateC}>Update Content</Button>
