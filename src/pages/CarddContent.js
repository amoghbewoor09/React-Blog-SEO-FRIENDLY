...
export default function CardbContent() {
  const { lastUpdatedD, setLastUpdatedD } = useContext(DateTimeContext);

  const updateTimeAndDateD = () => {
    const currentDateTime = moment().format('YYYY-MM-DD HH:mm:ss');
    setLastUpdatedD(currentDateTime);
  };
    ...
    <>
      ...
      <div className="blog-container" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', padding: '3rem' }}>
        <div className="header" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {lastUpdatedD && (<Typography variant="body2" color="primary"> Last Updated: {lastUpdatedD}</Typography>)}
        ...
        <div className="content" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Button variant="contained" style={{ marginTop: '2rem' }} onClick={updateTimeAndDateD}>Update Content</Button>
